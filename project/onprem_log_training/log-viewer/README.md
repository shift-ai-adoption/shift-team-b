...
| # | 画面 | アクセス先 | 役割 |
|---|---|---|---|
| 1 | **RoboMart（ECサイト）** | `http://<研修環境のホスト>:<PORT>/` | ログを発生させる側。商品を閲覧・操作するとログが出力されます |
| 2 | **Log Viewer（ダッシュボード）** | `http://<研修環境のホスト>:<LOG_VIEWER_PORT>/` | ログを可視化する側。①の操作結果がここに表示されます |

> **ホスト名とポート番号の確認方法**
> - ホスト名（アクセス先）は研修担当者から共有されます
> - ポート番号は `.env` の `PORT`（RoboMart）と `LOG_VIEWER_PORT`（Log Viewer）で決まります
> - 稼働中のポートは `docker ps` で確認できます
...
ブラウザで `http://<研修環境のホスト>:<LOG_VIEWER_PORT>/` を開きます。
