...
研修環境はすでに起動しています。以下の2つの画面をブラウザで開いてください。
**ホスト名（アクセス先）は研修担当者から共有されます。**

| # | 画面 | ポート | アクセス先 | 役割 |
|---|---|---|---|---|
| 1 | **RoboMart（ECサイト）** | **3002** | `http://<ホスト名>:3002/` | ログを発生させる側。商品を閲覧・操作するとログが出力されます |
| 2 | **Log Viewer（ダッシュボード）** | **8001** | `http://<ホスト名>:8001/` | ログを可視化する側。①の操作結果がここに表示されます |

> **ポート番号が上記と違う場合**
> ポート番号は `.env` の `PORT`（RoboMart）と `LOG_VIEWER_PORT`（Log Viewer）で決まります。
> 実際に稼働しているポートは `docker ps` で確認できます。
>
> ```
> onprem-log-training-client   0.0.0.0:3002->3002/tcp
> onprem-log-viewer            0.0.0.0:8001->8090/tcp
> ```
>
> Log Viewer は**コンテナ内部は常に8090**で、ホスト側の公開ポートのみ `LOG_VIEWER_PORT` で変わります。

### おすすめの確認手順

1. **①RoboMart** を開き、商品一覧から適当な商品の詳細ページを表示する
2. 特に **WalkyDog Mk2** の詳細ページを開くと、意図的にエラー（HTTP 500）が発生します
...
ブラウザで `http://<ホスト名>:8001/` を開きます（`LOG_VIEWER_PORT` のデフォルトは `8001`）。

`LOG_VIEWER_PORT` は `.env` で指定してください（環境のSG/ファイアウォールで開放済みのポートに合わせること）。
コンテナ内部は常に `8090` で待ち受けるため、変更するのはホスト側の公開ポートのみです。
